...
export function parseCsv<T = any>(text: string) {
  return Papa.parse<T>(text, { header: true, dynamicTyping: true, skipEmptyLines: true });
}

/** 在若干候选键里取第一个存在的值 */
export function getAny<T extends object, K extends string>(row: T, keys: K[]) {
  for (const k of keys) if (k in (row as any) && (row as any)[k] != null) return (row as any)[k];
  return undefined;
}
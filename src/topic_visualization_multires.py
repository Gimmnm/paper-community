...
try:
    ...
    from diagram2d import plot_scatter
except Exception as e:  # pragma: no cover
    raise ImportError(
        "无法导入 topic_modeling.py / diagram2d.py。请将本脚本放在 src/ 并从项目根目录运行。"
    ) from e
...
def main() -> None:
    ...
    for i, (r, mem_path) in enumerate(items):
        ...
        membership = tm.load_membership(mem_path, n_papers_expected=Y_umap.shape[0])

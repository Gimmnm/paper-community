...
def load_or_run_leiden_partition(
    ...
    *,
    ...
    if membership_path.exists():
        ...
        load_sec = max(float(time.perf_counter() - t0), 1e-9)
        if verbose:
...
def rebuild_sweep_summary_from_membership_dir(
    out_dir: Path,
    *,
    backup: bool = True,
    ndigits: int = DEFAULT_RESOLUTION_NDIGITS,
    ...
    """
    Recompute ``summary.npy`` + ``summary.csv`` from existing ``membership_r*.npy`` files only
    (no graph repartition). Use when membership files are a superset of ``summary.npy`` resolutions.
    """
    import shutil
    ...
    raw_rs: List[float] = []
    for p in out_dir.glob("membership_r*.npy"):
        m = pat.match(p.name)
        if not m:
            continue
        try:
            raw_rs.append(float(m.group(1)))
        except ValueError:
            continue
    if not raw_rs:
        raise FileNotFoundError(f"no membership_r*.npy under {out_dir}")
    keys = sorted({_round_resolution(float(r), ndigits=ndigits) for r in raw_rs})
    results: Dict[float, Dict[str, Any]] = {}
    for rk in keys:
        mp = out_dir / membership_filename(rk, ndigits=ndigits)
        if not mp.is_file():
            if verbose:
                print(f"[rebuild-summary] skip r={rk}: missing {mp.name}")
            continue
        mem = np.load(mp).astype(np.int32)
        rk_eff = _round_resolution(float(rk), ndigits=ndigits)
        results[rk_eff] = {
            "resolution": rk_eff,
            "membership": mem,
            "n_comm": int(np.unique(mem).size),
            "quality": float("nan"),
            "time": 0.0,
        }
    if len(results) < 2:
        raise ValueError(f"need at least 2 usable membership files, got {len(results)} under {out_dir}")
    summary = summarize_sweep_results(results)
    summary_path = out_dir / "summary.npy"
    if backup and summary_path.is_file():
        bak = out_dir / f"summary.npy.bak_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        shutil.copy2(summary_path, bak)
        if verbose:
            print(f"[rebuild-summary] backed up -> {bak.name}")
    np.save(summary_path, summary, allow_pickle=True)
    _write_summary_csv(out_dir / "summary.csv", summary)
    if verbose:
        print(f"[rebuild-summary] wrote {summary_path.name} + summary.csv, n_res={len(results)}")
    return summary

...
def choose_memberships_exact(
    all_items: list[tuple[float, Path]],
    resolutions: Iterable[float] | None,
    *,
    tol: float = 1e-6,
) -> list[tuple[float, Path]]:
    """显式指定分辨率时的匹配（与 topic_modeling_multi.select_memberships 容差一致）。"""
    ...
    for rr in req:
        for r, p in all_items:
            if abs(float(r) - float(rr)) <= tol:
                out.append((r, p))
                break
    uniq = {round(float(r), 10): (r, p) for r, p in out}

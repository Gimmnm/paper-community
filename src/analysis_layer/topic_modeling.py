...
│   └── data_store.pkl
├── src/
│   └── analysis_layer/topic_modeling.py   <-- 本文件实现
└── out/
    └── leiden_sweep/
        └── membership_r1.0000.npy

运行示例（在 project 根目录，推荐走 core）：
python src/core.py topic-model --k-topics 10 --resolution 1.0

或直接模块（需 PYTHONPATH=src）：
PYTHONPATH=src python -m analysis_layer.topic_modeling --k 10 --resolution 1.0

也可显式指定 membership 文件：
PYTHONPATH=src python -m analysis_layer.topic_modeling --k 10 --membership out/leiden_sweep/membership_r1.0000.npy
"""

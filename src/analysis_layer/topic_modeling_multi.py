...
推荐用法（在项目根目录）：
    python src/core.py topic-model-multi --k-topics 10

只跑部分分辨率：
    PYTHONPATH=src python -m analysis_layer.topic_modeling_multi --k 10 --r-min 0.5 --r-max 1.5 --step 0.1 --include 1.0

显式指定 membership 文件：
    PYTHONPATH=src python -m analysis_layer.topic_modeling_multi --k 10 \
...
# 直接复用 analysis_layer 下的单分辨率主题建模实现
try:
    from analysis_layer import topic_modeling as tm
except Exception as e:  # pragma: no cover
    raise ImportError(
        "无法导入 analysis_layer.topic_modeling。请确保从项目根目录运行且 PYTHONPATH=src。"
    ) from e
...
def run_one_resolution(
    *,
    ...
    df_comm = pd.read_csv(
        out_dir / "communities_topic_weights.csv",
        encoding="utf-8-sig",
        low_memory=False,
    )
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="可选：experiment-comparison-breakpoints 产出的 CSV；与 --breakpoint-run-id 联用，仅跑各算法 ~10 个断点分辨率",
    )
    p.add_argument("--breakpoint-run-id", type=str, default=None, help="CSV 中 run_id 列（如 leiden_cpm）")
    p.add_argument("--breakpoint-time-window", type=str, default="all", help="过滤 CSV 的 time_window 列")
...
def main() -> None:
    ...
    if getattr(args, "breakpoints_csv", None):
        ...
        rid = getattr(args, "breakpoint_run_id", None)
        if not rid:
            raise SystemExit("topic-model-multi: --breakpoint-run-id is required when using --breakpoints-csv")
        bp_path = Path(args.breakpoints_csv)
        ...
        if not rs_bp:
            raise SystemExit(f"topic-model-multi: no rows for run_id={rid!r} in {bp_path}")
        args.resolutions = rs_bp
        if verbose:
            print(f"[multi] breakpoints mode: {bp_path} run_id={rid} -> {len(rs_bp)} resolutions")
    ...
    for r, mem_path in selected:
        ...
        try:
            row = run_one_resolution(
                ...
                build_cfg=build_cfg,
                score_cfg=score_cfg,
                rep_papers_mode=str(args.rep_papers_mode),
                verbose=verbose,
            )
            rows.append(row)
